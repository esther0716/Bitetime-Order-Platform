...
The build emits **one HTML file per prerendered route plus a shell, and which one serves a route matters**. `vite build` produces the usual empty SPA shell, then `scripts/prerender.tsx` (an SSR build, run by the same `build` script) renders each marketing route in its `ROUTES` list to static markup: `/` is written over `dist/index.html`, `/pricing` becomes `dist/pricing.html`, and the untouched shell is kept as `dist/app.html`. `vercel.json` rewrites everything to **`/app.html`**, not `index.html`. Point that rewrite back at `index.html` and every storefront starts life with the marketing page's markup inside it.
...
Adding a marketing page therefore means adding it in **four** places: a `<Route>` in `AppRouter.tsx`, an entry in the prerender `ROUTES` list, a title/description in `src/routeMeta.ts`, and a rewrite in `apps/frontend/vercel.json`. Miss the prerender list and the page reaches a JS-less crawler as a blank document; miss `routeMeta.ts` or the rewrite and a test fails.
