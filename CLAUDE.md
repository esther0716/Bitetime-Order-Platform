...
The build emits **one HTML file per prerendered route plus a shell, and which one serves a route matters**. `vite build` produces the usual empty SPA shell, then `scripts/prerender.tsx` (an SSR build, run by the same `build` script) renders each route in its `ROUTES` list to static markup: `/` is written over `dist/index.html`, `/pricing` becomes `dist/pricing.html`, and the untouched shell is kept as `dist/app.html`. The list is **every URL in `public/sitemap.xml`** — the four marketing pages plus `/terms`, `/privacy`, `/merchant/signup` and `/merchant/login`, which are app pages and are prerendered for a narrower reason: served the shell, a crawled sitemap URL gets the HOMEPAGE's title and description and no canonical at all. `vercel.json` rewrites everything to **`/app.html`**, not `index.html`. Point that rewrite back at `index.html` and every storefront starts life with the marketing page's markup inside it.
