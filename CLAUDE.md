...
Fonts are requested from `index.html`, never from an `@import` in `src/index.css`. Inside the stylesheet the request is invisible to the preload scanner and is made only after the whole CSS bundle has downloaded and parsed, which puts Google's stylesheet and then the font file — two cold origins — in series behind a render-blocking resource. The Latin faces stay render-blocking (they are the page); the Chinese faces load `media="print"` and flip to `all` on load, so they never hold first paint.

The build emits **one HTML file per prerendered route plus a shell, and which one serves a route matters**. `vite build` produces the usual empty SPA shell, then `scripts/prerender.tsx` (an SSR build, run by the same `build` script) renders each marketing route in its `ROUTES` list to static markup: `/` is written over `dist/index.html`, `/pricing` becomes `dist/pricing.html`, and the untouched shell is kept as `dist/app.html`. `vercel.json` rewrites everything to **`/app.html`**, not `index.html` — Vercel checks the filesystem before rewrites, so a prerendered path is served by its own file and never reaches the rule. Point that rewrite back at `index.html` and every storefront starts life with the marketing page's markup inside it.

Adding a marketing page means adding it in **three** places: a `<Route>` in `AppRouter.tsx`, an entry in that `ROUTES` list, and a title/description in `src/routeMeta.ts`. Miss the prerender list and the page still works — it just falls through to `app.html` and reaches a JS-less crawler as a blank document. Miss `routeMeta.ts` and the build throws rather than shipping a page titled as the homepage.
