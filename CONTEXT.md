...
> **Not all of this is built yet.** The public and merchant lookups already withhold the redeemer list — that shipped on its own, as a fix, and is described in the present tense below. Everything else here (`per_customer_limit`, `expires_at`, `min_order`, `voucher_redemptions`, the renamed refusal codes, soft delete) is the settled design for #241, recorded in [ADR 0019](docs/adr/0019-voucher-redemptions-are-rows-not-a-list.md) and [ADR 0020](docs/adr/0020-two-stripe-voucher-restrictions-bitetime-cannot-key.md), and has no migration behind it.
