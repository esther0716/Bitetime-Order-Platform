...
// Same reason as faq.test.ts: both language fields are strings, so an entry carrying English in
// its Chinese slot type-checks perfectly and ships inside a Chinese page. Nothing here tests
// rendering.
...
describe('FEATURES content', () => {
  ...
  it('gives every entry a title and a body in both languages', () => {
    for (const [i, entry] of FEATURES.entries()) {
