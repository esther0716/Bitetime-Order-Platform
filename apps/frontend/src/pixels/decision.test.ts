...
const on = { configured: true, onMarketingPath: true }
const off = { configured: true, onMarketingPath: false }

describe('on a marketing page, with a pixel configured', () => {
...
describe('off a marketing page', () => {
  ...
  // the load is the third-party request and the advertising cookie.
...
describe('with no pixel configured', () => {
  ...
  it('does nothing whatever the visitor answered', () => {
    for (const choice of [null, 'accepted', 'rejected'] as const) {
      expect(pixelDecision({ configured: false, onMarketingPath: true, choice }))
