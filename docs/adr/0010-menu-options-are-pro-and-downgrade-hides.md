...
- **Refusing any body containing groups**, mirroring the promo gate — forces the dashboard to send explicit nulls to save anything else, which is how an ex-Pro shop editing a product's **name** clears its sale today. That bug is not worth reproducing.
...
- **The comparison gate is stricter than the promo gate**, so the two now behave differently on the same endpoint: a Basic shop can rename a product without losing its groups but not without losing its promo. That inconsistency is a bug in the promo gate, and should be fixed to match rather than tolerated as symmetry.
